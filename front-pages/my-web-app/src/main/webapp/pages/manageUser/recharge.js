...
(function() {
	...
	Recharge.prototype = {
		...
		initHtml : function(){			
			var html='';
			html+='<div title="帐户充值" id="myPanel"><form id="form1">';
		...
		initLayoutFrom : function(){
			...
			var fields =[
							...
							{ label: '充值金额', name: 'storeName', validate: {required:true,minlength:2,maxlength:60}, type: 'text', group: '充值信息', groupicon: root+'/images/group.gif' },
							{ label: '银行信息', name: 'storeType', validate: {required:true,minlength:1,maxlength:8}, newline: false, type: 'combobox', 
			...
			if(parent.window.merchantList.cache.subPageId==='add'){
				fields.push({ label: '<a href="javascript:window.recharge.secretKey()">生成秘钥对</a>', 
					labelWidth:'500', name: '___span1', type:'span',  rightToken:' '});
				fields.push({ label: '<input id="check1" name="check1" class="check1" type="checkbox" checked="checked" />&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:window.recharge.showClause()">我同意【服务协议】和【隐私条款】</a>', 
					name: '___span2', type:'span', labelWidth:'500', rightToken:' ', group: '其它信息', groupicon: root+'/images/group.gif'});
			}else{
				fields.push({ label: '<a href="javascript:window.recharge.secretKey()">生成新秘钥对</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:window.recharge.reset()">恢复原密钥对</a>', 
					labelWidth:'500', name: '___span1', type:'span',  rightToken:' '});
			}
			...
			if(storeName != existStoreName){
				var pp = {'storeName':storeName, 'message':'storeName'};
				 JAjax(that.config.chackerUrl, pp, 'json', function(data){
					 error=data;
		        }, function(){JSWK.clewBox('提交数据时发生异常','clew_ico_fail',failShowTime);}, false); 
			}
			if(!error){
				alert('商户名称已存在');
				return;
