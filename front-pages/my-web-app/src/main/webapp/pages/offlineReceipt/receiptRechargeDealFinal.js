(function() {
	var DealFinalList = function() {
		this.init();
	};
	DealFinalList.prototype = {
		config : {
...
//						console.log('date: ' + dealDate.operTime);
						...
						$("#dealApprovalResult").text(dealDate.approvalResult == '1' ? "同意" : "不同意");//审批结果
					}
				});
			});
...
//					alert('stepNo: ' + rechargeData.stepNo + "\t" + 'approvalResult: ' +approvalResult);
					if(rechargeData.stepNo=='4302'){
							if(approvalResult=='不同意'&&(approvalSuggest==null||approvalSuggest=="")){
								hideMask();
								parent.window.receiptList.alert('请填写备注', 'clew_ico_fail');
								return false;
							}else{								
...
//				console.log('data.channel： ' + data.channel + 'data.subChannel: ' + data.subChannel + 'data.merchantCode: ' + data.merchantCode);
	...
	$(document).ready(function() {
		window.dealFinalList = new DealFinalList();
