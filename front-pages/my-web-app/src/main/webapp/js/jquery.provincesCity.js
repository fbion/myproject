$.fn.ProvinceCity = function(){
	var _self = this;
	//定义3个默认值
	//_self.data("country",["请选择", "请选择"]);
	_self.data("province",["请选择所在省", ""]);
//	_self.data("city1",["请选择所在市", ""]);
	//_self.data("city2",["请选择", "请选择"]);
	//插入3个空的下拉框
	//_self.append("<select></select>");
	_self.append("<select id='province' style='width:128px;higth:50px'></select>&nbsp;&nbsp;&nbsp;&nbsp;");
//	_self.append("<select id='city' style='width:128px;higth:50px'></select> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
	//分别获取3个下拉框
	//var $sel0 = _self.find("select").eq(0);
...
//	var $sel2 = _self.find("select").eq(1);
	...
	/*if(_self.data("country")){
		$sel0.append("<option value='"+_self.data("country")[1]+"'>"+_self.data("country")[0]+"</option>");
	}
	$.each( GP , function(index,data){
		$sel0.append("<option value='"+data+"'>"+data+"</option>");
	});*/
	//默认省级下拉
...
//	}
	...
	$sel0.change(function(){
		...
		if(index0 == 0){	//当选择的为 “请选择” 时
			if(_self.data("province")){
				$sel2.append("<option value='"+_self.data("province")[1]+"'>"+_self.data("province")[0]+"</option>");
			}
			if(_self.data("city1")){
				$sel2.append("<option value='"+_self.data("city1")[1]+"'>"+_self.data("city1")[0]+"</option>");
			}
		} else {
			$.each( GT[index0-1] , function(index,data){
				$sel1.append("<option value='"+data+"'>"+data+"</option>");
			});
			$.each( GC[index0-1][0] , function(index,data){
