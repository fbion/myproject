...
public class RechargeController extends BaseController {
	...
	@Resource(name = "rechargeService", description = "service充值接口")
	private RechargeService rechargeService;
	@Resource(name = "approveService", description = "service充值接口")
	private ApproveService approveService;
	
//	 @Resource(name = "smsService", description = "service短信接口")
//	private SmsService smsService;
//	
//	@Resource(name = "accountService", description = "service账户接口")
//	private IAccountService accountService;
	...
	public PaginationResult<?> queryList(DynamicSqlParameter requestParam) {
		try {
			result = rechargeService.queryByPage(requestParam);
	...
	public String isVoucherUnique(DynamicSqlParameter requestParam) {
		try {
			return rechargeService.isVoucherUnique(requestParam);
		} catch (UPPException e) {
	...
	@ResponseBody
	public void upload(
			@RequestParam(required = false, value = "fileName") final MultipartFile file,HttpServletResponse response) {
		String flag = "0";
		File f = null;
		try {			
			...
			String imgName = rechargeService.uploadFile(bean);
			...
			result.setMessage("线下充值上传汇款凭证异常!");
			logger.error("线下充值上传汇款凭证异常!", e);
			this.sendAjaxInfo("线下充值上传汇款凭证异常!", response);
		}finally{
			try{
				if(f!=null)f.delete();
			}catch(Exception e){
	...
	public PaginationResult<?> add() {
		try {
			...
			result = rechargeService.add((TradeApplyDto) model, loginUser);
		} catch (Exception e) {
	...
	public String getNum2CNString(DynamicSqlParameter requestParam) {
		String temp = "";
		try {
			temp = rechargeService.getNum2CNString(requestParam.getEqual().get(
					"tradeAmount"));
	...
	public TradeApplyDto queryById(@RequestParam String id) {
		...
		try {
			trade = rechargeService.queryById(id);
		} catch (UPPException e) {
	...
	public Object queryRechargeApplyVoucher(@RequestParam String applyId) {
		...
		try {
			VoucherDto bean = rechargeService
...
//		return voucherFileName;
	...
	public PaginationResult<TradeApplyDto> addApproval(){
		...
		try {
			...
			OfflineRechargeApply bean = rechargeService.getRechargeApplyById(dto.getId());
...
//			applyDto.setId(bean.getId());
			...
			if("0".equals(dto.getIdentifier())){
				if("同意".equals(dto.getApprovalResult())){
					if(tradeAmount<Long.parseLong(maxAmount)){//判断出纳审核的金额是否小于10万
//						dto.setStepNo("4303");
...
//			approvalDto.setApprovalSuggest("同意");
			...
			if ("0".equals(dto.getIdentifier())) {
				...
				this.rechargeService.addVoucher(voucherDto);
			}
			
			this.rechargeService.addApproval(approvalDto);
...
//			rechargeService.modifyRechargeApply(bean,approvalBean);
...
//	}
	...
	public PaginationResult<TradeApplyDto> editStepNo() {
		...
		PaginationResult<TradeApplyDto> result = new PaginationResult<TradeApplyDto>();
		try {
			result = rechargeService.editStepNo((TradeApplyDto) model, user);
		} catch (UPPException e) {
	...
	public PaginationResult<TradeApplyDto> modify() {
		...
		PaginationResult<TradeApplyDto> result = new PaginationResult<TradeApplyDto>();
		try {
			result = rechargeService.update((TradeApplyDto) model, user);
			result.setMessage(Converter.OP_SUCCESS);
		} catch (UPPException e) {
			result.setMessage(Converter.OP_FAILED);
...
//			}
	...
	public void exportExcelAll(DynamicSqlParameter requestParam){
		try {
			...
			rechargeService.exportExcel(requestParam);
		} catch (Exception e) {
