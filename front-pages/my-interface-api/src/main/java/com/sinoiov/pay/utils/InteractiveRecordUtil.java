...
public class InteractiveRecordUtil {
	private static Log logger = LogFactory.getLog(InteractiveRecordUtil.class);
	...
	public static void saveRecord(final String jsonData, final Object returnData, 
						final String storeCode,final String operCode) {
		TaskPool.named(__INTERACTION_LOG_EXECUTOR_TASK_POOL__)
				.setPoolSize(4).submit(new Runnable() {

			@Override
			public void run() {
				doSaveRecord(jsonData, returnData, storeCode, operCode);
			}
			
		});
	}
	
	public static void doSaveRecord(String jsonData,Object returnData,String storeCode,String operCode) {
		try{
			...
			interactiveRecord.setJsonData(jsonData);
			if(returnData!=null){
				JSONObject jsonMap = JSONObject.fromObject(returnData);						
				String returnJson = jsonMap.toString();
	...
	public static void saveCallbackOrderRecord(OrderRecord record) {
		try {
			mongoService.setDatasource("LOGS");
