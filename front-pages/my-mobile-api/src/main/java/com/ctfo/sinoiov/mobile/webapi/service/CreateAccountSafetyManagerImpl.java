 ...
 * @author sunchuanfu
 */
@Service("createAccountSafetyManagerImpl")
public class CreateAccountSafetyManagerImpl implements IJsonService {
	protected static final Log logger = LogFactory.getLog(CreateAccountSafetyManagerImpl.class);
	...
	public Body execute(Parameter request, Object... obj) throws ClientException {
		...
		try {
			...
			UppResult uppResult = PayManagerUtil.invokeUPP(req, "UPP_CREATE_ACCOUNT_SAFETY");
			...
			logger.error("创建安全问题时报错！", e);
			throw new ClientException(PayError.E240001, "创建安全问题时报错!");
		}
