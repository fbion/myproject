 ...
 * @author sunchuanfu
 */
@Service("checkAccountSafetyManagerImpl")
public class CheckAccountSafetyManagerImpl implements IJsonService {
	protected static final Log logger = LogFactory.getLog(CheckAccountSafetyManagerImpl.class);
	...
	public Body execute(Parameter request, Object... obj) throws ClientException {
		...
		try {
			...
			UppResult uppResult = PayManagerUtil.invokeUPP(req, "UPP_CHECK_ACCOUNT_SAFETY");
			...
			logger.error("验证安全问题时报错！", e);
			throw new ClientException(PayError.E240001, "验证安全问题时报错!");
		}
	...
	public void validate(Parameter request) throws ClientException {
		...
		// 验证3个安全问题和对应答案是否不全(只验证一个问题)
		if ((StringUtils.isNotBlank(req.getSecurityQuestion1()) && StringUtils.isNotBlank(req.getSecurityAnswer1()))
				|| (StringUtils.isNotBlank(req.getSecurityQuestion2()) && StringUtils.isNotBlank(req
						.getSecurityAnswer2()))
				|| (StringUtils.isNotBlank(req.getSecurityQuestion3()) && StringUtils.isNotBlank(req
						.getSecurityAnswer3()))) {
		} else {
