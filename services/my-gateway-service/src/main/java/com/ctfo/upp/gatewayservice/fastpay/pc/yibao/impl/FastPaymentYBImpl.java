...
import com.ctfo.upp.gateway.fastpay.pc.yibao.request.domain.YBFastPayRequestDomain;
import com.ctfo.upp.gateway.fastpay.pc.yibao.response.domain.YBFastPayRequestResponseDomain;
...
public class FastPaymentYBImpl extends AbstractFastPaymentYB implements FastPayment {
	...
	public FastPayDomain fastPayment(FastPayDomain paymentDomain) throws UPPException {
		...
		try{
			...
			if (!(paymentDomain instanceof YBFastPayRequestDomain))
				throw new IllegalStateException("PC端调用一键支付传递的请求参数bean无效.");
			...
			try {
				...
				throw new UPPException("将一键支付PC端请求参数转换为JSON格式时产生错误，原因：", e);
			}
		
			logger.info(String.format("PC端发送易宝一键支付参数(明文)：paramsJSON=[%s]", paramsJSON));
			...
			logger.info("PC端发送易宝一键支付参数(密文)：data="+data+"&encryptKey"+encryptKey);
			...
			logger.error("PC端发送易宝参数异常：", e);
			throw new UPPException(e.getLocalizedMessage());
		}finally{
			String command = ConfigUtil.getValue("YB_PC_FAST_PAY_REQUEST_URL");
	...
	protected String buildRequestURL(final String merchantAccount, final String data, final String encryptKey) {
		final String requestURL = ConfigUtil.getValue("YB_PC_FAST_PAY_REQUEST_URL");
		if (logger.isDebugEnabled())
	...
	static private void fillSpecialPropertyValue(Map<String, Object> paramsMap) {
		if (paramsMap == null || paramsMap.isEmpty())
			return;
		paramsMap.put(CALLBACK_URL_KEY, ConfigUtil.getValue("YB_PC_FAST_PAY_GATEWAY_CALLBACK_URL"));
	}
