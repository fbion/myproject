...
public class FastPaymentClearingDataYBImpl extends AbstractFastPaymentYB implements FastPaymentClearingData {
	private final Log logger = LogFactory.getLog(getClass());

	@Override
	public FastPayDomain fastPaymentGetClearingData(FastPayDomain requestDomain) throws UPPException {
		// TODO Auto-generated method stub
		objectNotNull(requestDomain);
		if (!(requestDomain instanceof YBClearingDataRequestDomain))
			throw new IllegalStateException("传递的获取消费清算对账单请求对象参数bean无效.");
		fillDefaultPropertyValue(requestDomain);
		checkObjectField(requestDomain);
		final Map<String, Object> paramsMap = object2Map(requestDomain);
		...
		try {
			...
			throw new UPPException("将获取消费清算对账单请求对象参数转换为JSON格式时产生错误，原因：", e);
		}
		if (logger.isDebugEnabled())
			logger.debug(String.format("clearing data paramsJSON=[%s]", paramsJSON));
		final String merchantAesKey = buildMerchantAesKey();
		final String data = AES.encryptToBase64(paramsJSON, merchantAesKey);
		if (logger.isDebugEnabled())
			logger.debug(String.format("clearing data [data] parameter=[%s]", data));
	...
	protected String buildRequestURL(final String merchantAccount, final String data, final String encryptKey) {
		final String requestURL = ConfigUtil.getValue("YB_COMMON_FAST_PAY_CLEARING_DATA_BILLS_REQUEST_URL");
		if (logger.isDebugEnabled())
			logger.debug(String.format("The clearing data request URL is:%s", requestURL));
		...
		if (logger.isDebugEnabled())
			logger.debug(String.format("The clearing data request full URL is:%s", requestFullURL));
		return requestURL;
	}
	...
	protected FastPayDomain buildResponse(final String response) throws UPPException {
		...
		checkResponsed(plaintextResult);
		if (logger.isDebugEnabled())
			logger.debug("plaintextResult=" + plaintextResult);
